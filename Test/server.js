...
// Endpoint to update username and profile picture
app.post('/update-profile', upload.single('profile_image'), (req, res) => {
  const userId = req.body.userId;
  const username = req.body.username;
  const profile_image = req.file ? req.file.filename : null;  // Save image filename if uploaded

  if (!userId || !username) {
    return res.status(400).json({ message: 'User ID and new name are required' });
  }

  // SQL query to update user info, if profileImage is uploaded
  let query = `UPDATE users SET username = ?, profile_image = ? WHERE id = ?`;
  let params = [username, profile_image, userId];

  // If no new profile image is provided, only update the username
  if (!profile_image) {
